# File      :_2Dsteady_TopCapDrive_SUS.py
# Time      :2022/9/15--10:29
# Author    :JF Li
# Version   :python 3.7
...
def phyloss(u, v, p, mu, rho, init_u, delta_xy):
    ...
    # sus boundary encoding of u, v
    u, v = sus_boundary(u, v)

    ave_u = (u[0, 0, 2:-2, 1:-2] + u[0, 0, 2:-2, 2:-1]) / 2
    ave_v = (v[0, 0, 1:-2, 2:-2] + v[0, 0, 2:-1, 2:-2]) / 2
    ...
    max_E = torch.max(
        ...
        dim = 0)[0]
    one = torch.ones_like(loss_u).to(device)

    loss_u += (one + 0.5 * max_W[:, 1:] + 1.5 * max_W[:, :-1]) * u[0, 0, 2:-2, 1:-3] + \
              (one + 1.5 * max_E[:, 1:] + 0.5 * max_E[:, :-1]) * u[0, 0, 2:-2, 3:-1] + \
              (one + 0.5 * max_S[1:, :] + 1.5 * max_S[:-1, :]) * u[0, 0, 1:-3, 2:-2] + \
              (one + 1.5 * max_N[1:, :] + 0.5 * max_N[:-1, :]) * u[0, 0, 3:-1, 2:-2] - \
              0.5 * (max_W[:, :-1] * u[0, 0, 2:-2, :-4] +
                     max_E[:, 1:] * u[0, 0, 2:-2, 4:] +
                     max_S[:-1, :] * u[0, 0, :-4, 2:-2] +
                     max_N[1:, :] * u[0, 0, 4:, 2:-2]) - \
              (0.5 * (max_W[:, 1:] + max_E[:, :-1] + max_S[1:, :] + max_N[:-1, :]) +
               max_W[:, :-1] + max_E[:, 1:] + max_S[:-1, :] + max_N[1:, :] +
               p_delta_u[:, 1:] - p_delta_u[:, :-1] + p_delta_v[1:, :] - p_delta_v[:-1, :] +
               4 * one) * u[0, 0, 2:-2, 2:-2]
    loss_v += (one + 0.5 * max_W[:, 1:] + 1.5 * max_W[:, :-1]) * v[0, 0, 2:-2, 1:-3] + \
              (one + 1.5 * max_E[:, 1:] + 0.5 * max_E[:, :-1]) * v[0, 0, 2:-2, 3:-1] + \
              (one + 0.5 * max_S[1:, :] + 1.5 * max_S[:-1, :]) * v[0, 0, 1:-3, 2:-2] + \
              (one + 1.5 * max_N[1:, :] + 0.5 * max_N[:-1, :]) * v[0, 0, 3:-1, 2:-2] - \
              0.5 * (max_W[:, :-1] * v[0, 0, 2:-2, :-4] +
                     max_E[:, 1:] * v[0, 0, 2:-2, 4:] +
                     max_S[:-1, :] * v[0, 0, :-4, 2:-2] +
                     max_N[1:, :] * v[0, 0, 4:, 2:-2]) - \
              (0.5 * (max_W[:, 1:] + max_E[:, :-1] + max_S[1:, :] + max_N[:-1, :]) +
    ...
    mass += rho * (v[0, 0, 3:-1, 2:-2] - v[0, 0, 1:-3, 2:-2]) / 2 * delta_xy / mu
    mass += rho * (u[0, 0, 2:-2, 3:-1] - u[0, 0, 2:-2, 1:-3]) / 2 * delta_xy / mu
...
def train(coor, value, model, epoch, mu, rho, init_u, delta_xy, lr = 0.01):
    ...
    for i in range(epoch):
        ...
        loss_u, loss_v, mass = phyloss(u, v, p, mu, rho, init_u, delta_xy)
...
if __name__ == '__main__':
    ...
    # error = train(coor, value, model, epoch, mu, rho, init_u, delta_xy)
    ...
    # test
    pred_FUS = test(coor, value, model, init_u, './models/data3_SUS/model_200000.pt')
