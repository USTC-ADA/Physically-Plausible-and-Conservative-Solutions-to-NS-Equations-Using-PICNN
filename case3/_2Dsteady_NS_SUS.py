...
def phyloss(u, v, p, mu, rho, init_u, delta_xy, inlet, outlet, scheme = 'FUS'):
    ...
    max_E = torch.max(
        ...
        dim = 0)[0]
    a_N = (A(torch.abs(p_delta_v), scheme = scheme) + max_N) * mu / delta_xy
    a_S = (A(torch.abs(p_delta_v), scheme = scheme) + max_S) * mu / delta_xy
    a_W = (A(torch.abs(p_delta_u), scheme = scheme) + max_W) * mu / delta_xy
    a_E = (A(torch.abs(p_delta_u), scheme = scheme) + max_E) * mu / delta_xy
    loss_u += a_N[1:, :] * (u[0, 0, 2:, 1:-1] - u[0, 0, 1:-1, 1:-1]) - rho * ave_v[1:, :] * u[0, 0, 1:-1, 1:-1]
    loss_u += a_S[:-1, :] * (u[0, 0, :-2, 1:-1] - u[0, 0, 1:-1, 1:-1]) + rho * ave_v[:-1, :] * u[0, 0, 1:-1, 1:-1]
    loss_u += a_W[:, :-1] * (u[0, 0, 1:-1, :-2] - u[0, 0, 1:-1, 1:-1]) + rho * ave_u[:, :-1] * u[0, 0, 1:-1, 1:-1]
    loss_u += a_E[:, 1:] * (u[0, 0, 1:-1, 2:] - u[0, 0, 1:-1, 1:-1]) - rho * ave_u[:, 1:] * u[0, 0, 1:-1, 1:-1]
    loss_v += a_N[1:, :] * (v[0, 0, 2:, 1:-1] - v[0, 0, 1:-1, 1:-1]) - rho * ave_v[1:, :] * v[0, 0, 1:-1, 1:-1]
    loss_v += a_S[:-1, :] * (v[0, 0, :-2, 1:-1] - v[0, 0, 1:-1, 1:-1]) + rho * ave_v[:-1, :] * v[0, 0, 1:-1, 1:-1]
    loss_v += a_W[:, :-1] * (v[0, 0, 1:-1, :-2] - v[0, 0, 1:-1, 1:-1]) + rho * ave_u[:, :-1] * v[0, 0, 1:-1, 1:-1]
    loss_v += a_E[:, 1:] * (v[0, 0, 1:-1, 2:] - v[0, 0, 1:-1, 1:-1]) - rho * ave_u[:, 1:] * v[0, 0, 1:-1, 1:-1]

    loss_u += (p[0, 0, 1:-1, :-2] - p[0, 0, 1:-1, 2:]) / 2
    loss_v += (p[0, 0, :-2, 1:-1] - p[0, 0, 2:, 1:-1]) / 2

    # Mass Equation
    mass = torch.zeros(p.shape[2] - 2, p.shape[3] - 2).to(device)
    mass += rho * (v[0, 0, 2:, 1:-1] - v[0, 0, :-2, 1:-1]) / 2
    mass += rho * (u[0, 0, 1:-1, 2:] - u[0, 0, 1:-1, :-2]) / 2

    # compute sus
    u, v = sus_boundary(u, v)
    D = mu / delta_xy
    loss_u += (u[0, 0, 2:-2, 1:-3] * (max_W[:, 1:] + max_W[:, :-1]) +
               u[0, 0, 2:-2, 3:-1] * (max_E[:, 1:] + max_E[:, :-1]) +
               u[0, 0, 1:-3, 2:-2] * (max_S[1:, :] + max_S[:-1, :]) +
               u[0, 0, 3:-1, 2:-2] * (max_N[1:, :] + max_N[:-1, :]) -
               u[0, 0, 2:-2, 2:-2] * (max_W[:, 1:] + max_E[:, :-1] + max_S[1:, :] + max_N[:-1, :]) -
               u[0, 0, 2:-2, :-4] * max_W[:, :-1] -
               u[0, 0, 2:-2, 4:] * max_E[:, 1:] -
               u[0, 0, :-4, 2:-2] * max_S[:-1, :] -
               u[0, 0, 4:, 2:-2] * max_N[1:, :]) * D * 0.5
    loss_v += (v[0, 0, 2:-2, 1:-3] * (max_W[:, 1:] + max_W[:, :-1]) +
               v[0, 0, 2:-2, 3:-1] * (max_E[:, 1:] + max_E[:, :-1]) +
               v[0, 0, 1:-3, 2:-2] * (max_S[1:, :] + max_S[:-1, :]) +
               v[0, 0, 3:-1, 2:-2] * (max_N[1:, :] + max_N[:-1, :]) -
               v[0, 0, 2:-2, 2:-2] * (max_W[:, 1:] + max_E[:, :-1] + max_S[1:, :] + max_N[:-1, :]) -
               v[0, 0, 2:-2, :-4] * max_W[:, :-1] -
               v[0, 0, 2:-2, 4:] * max_E[:, 1:] -
               v[0, 0, :-4, 2:-2] * max_S[:-1, :] -
               v[0, 0, 4:, 2:-2] * max_N[1:, :]) * D * 0.5
...
def train(coor, value, model, epoch, mu, rho, init_u, delta_xy, inlet, outlet, scheme = 'FUS', lr = 0.001):
    optimizer = optim.Adam(model.parameters(), lr = lr)
    scheduler = StepLR(optimizer, step_size = 50000, gamma = 0.5)

    # load previous model
    # model, optimizer, scheduler = load_checkpoint(model, optimizer, scheduler, save_dir = './models/model_200000.pt')
...
def plotting(coor, value, pred, fig_path):
    ...
    pred_p_min = 0.4  # min(pred_p.flatten())
    pred_p_max = 1.3  # max(pred_p.flatten())
...
if __name__ == '__main__':
    ...
    # error = train(coor, value, model, epoch, mu, rho, init_u, delta_xy, inlet, outlet, scheme = scheme)
    ...
    # test
    pred_FUS = test(coor, value, model, delta_xy, init_u, inlet, outlet, './models/data4_SUS/model_200000.pt')
